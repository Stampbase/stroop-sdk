...
- Keep public APIs deterministic and fully typed.
- Never collect telemetry by default.
- Do not require an API key for core open resolver functionality unless the
  architecture explicitly changes.
- Distinguish mainnet and testnet explicitly in every public API surface.
- Never silently send private user information to a hosted service.
- Document fallback behaviour, including what happens when hosted
  infrastructure is unavailable.
- Prefer direct protocol reads where appropriate, so consumers are not forced
  to depend on Stroop-operated infrastructure.
